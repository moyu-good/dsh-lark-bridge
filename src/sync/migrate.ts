...
import { maskSecret, readSettings, syncDir } from './settings-store.ts'
import type { ProfileManifest } from './profile-manifest.ts'
import { readProfileManifest } from './profile-manifest.ts'
import type { SyncPlan, InstallStep } from './plugin-sync.ts'
...
export const SECRET_KEYS = ['appSecret'] as const
...
export interface MigrationFile {
  ...
  exportedAt: string
  from: { profile: string; form: string; host: string }
  settings: SharedSettings
  /** Plugin lists per profile: installed packages + active bundles. */
  profiles: Record<string, { dependencies: Record<string, string>; bundles: string[] }>
...
export function migrationFilePath(home?: string): string {
 ...
 */
export function resolveMigrationFile(name: string | undefined, home?: string): string {
  const dir = syncDir(home)
  if (name === undefined || name === '') return path.join(dir, 'migrate.json')
...
export function validateMigration(parsed: unknown): MigrationFile {
  if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('迁移文件不是 JSON 对象')
  }
  const doc = parsed as Record<string, unknown>
  if (doc.kind !== MIGRATION_KIND) throw new Error(`kind 不是 \`${MIGRATION_KIND}\`——这不是桥的迁移文件`)
  if (doc.version !== MIGRATION_VERSION) throw new Error(`版本 ${String(doc.version)} 不受支持（当前 ${MIGRATION_VERSION}）`)
  const from = doc.from as MigrationFile['from'] | undefined
  if (from === undefined || typeof from.host !== 'string') throw new Error('缺 from.host')
  ...
  return doc as unknown as MigrationFile
}

/** Read + validate a migration document from the sync directory. */
export async function readMigration(name: string | undefined, home?: string): Promise<MigrationFile> {
...
export function buildImportPlan(
  local: ProfileManifest | null,
  profile: string,
  imported: TraveledProfile,
): SyncPlan {
 ...
 */
export function crossHostWarning(file: MigrationFile): string | null {
