...
import { syncDir } from './settings-store.ts'
...
export function accountsFile(home?: string): string {
  return syncDir(home) + '/' + FILE_NAME
}

/** Read the book, or an empty one when absent/corrupt. */
export async function readAccounts(home?: string): Promise<AccountBook> {
  try {
    const raw = await fsp.readFile(accountsFile(home), 'utf8')
    const parsed = JSON.parse(raw) as Partial<AccountBook>
...
export async function writeAccounts(book: AccountBook, home?: string): Promise<void> {
