...
#   （后续发现继续追加在这里）
set -u
DSH=/mnt/d/PROJECT/deepseek-harness/apps/cli/lib/bin.js
...
run_eval() {
  ...
  local out
  out=$(cd /mnt/d/PROJECT/deepseek-harness && timeout 240 node "$DSH" --profile headless --patch "$PATCH" "$prompt" 2>&1 | tail -1)
  if [[ "$out" == *"$expect"* ]]; then
    echo "✅ $name → \"$out\""
    pass=$((pass+1))
  else
