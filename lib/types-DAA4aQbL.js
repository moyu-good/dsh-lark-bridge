...
async function claimIfActiveStale(ctx, known) {
	...
	if (Object.entries(latest.devices ?? {}).filter(([id, entry]) => id !== latest.activeDeviceId && Date.now() - entry.lastSeen < 18e4).map(([id]) => id).some((id) => id < identity.deviceId)) return false;
...
async function devicesReply(ctx) {
	...
	if (arbitration !== null) {
		...
		for (const [id, entry] of Object.entries(arbitration.devices ?? {})) {
			...
			lines.push(`  - \`${entry.name}\`（${id}）${entry.form} v${entry.version} —— ${online ? `在线（${Math.round(age / 1e3)}s 前）` : `离线（${Math.round(age / 6e4)}min 前）`}${activeTag}`);
		}
	}
	try {
			...
			if (arbitration !== null) {
				...
				if (arbitration.activeDeviceId !== myIdentity.deviceId) {
					...
					if (!(syncCtx !== void 0 && await claimIfActiveStale(syncCtx, arbitration))) {
						await port.send(msg.chatId, { markdown: `↪️ 活跃设备是 **${arbitration.activeName}**，本端已退避。如需在本机接管，请发 \`/bot activate\`。` }).catch(reportSendFailure);
						return;
					}
