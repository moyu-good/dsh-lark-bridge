...
* @returns the markdown listing.
*/
function helpText(commands, agent, locale = "zh") {
	const own = [
...
*/
async function runCommandLine(line, agent, commands, signal, presets = void 0, persistence = void 0, chatId = void 0, deniedTools = void 0, schedules = void 0, audits = void 0, config = void 0, sessionPresets = void 0, sessionQuery = void 0, jobs = void 0, feedback = void 0, lastAssistantMessageId = void 0, tokenMeter = void 0) {
	...
	if (name === "help") return {
		reply: helpText(commands, agent, config?.locale ?? "zh"),
		resolved: true
	};
...
function subagentLine(descriptor) {
	return `🧑💻 ${descriptor.mode === "continuable" ? "可续子任务" : "子任务"}${descriptor.label === void 0 ? "" : `「${descriptor.label}」`} 已启动`;
}
...
function retryLine(retry) {
	if (retry.retry !== 1) return void 0;
	return `⚠️ 模型调用失败，正在重试${retry.maxRetries === void 0 ? "" : `（最多 ${retry.maxRetries} 次）`}…`;
}
...
function composeChatAgent(agentCtx, config, extraTools = [], runtimeDenied = void 0) {
	...
	prompt?.section({
		...
		order: 160,
		text: "the personal governance layer治理层（跨 bot 共享进度）：所有任务/事项先查共享进度再动手，有实质进展或完成时更新任务状态并留痕。统一入口 = the personal governance layer（/path/to/projects/the personal governance layer/），命令：python3 /path/to/projects/the personal governance layer/scripts/task_status.py --as dsh list （查）/ update <ID> --status ... --note ...（更新）/ done <ID>（完成），数据在 data/tasks.json + data/任务进度.md，与 hermes/codex/pi 互通。新增事项可用 python3 /path/to/projects/the personal governance layer/sansheng.py dispatch --input \"...\" 立案归部。"
	});
}
/**
...
function installBridge(ctx, config, port, notify, authorization) {
	...
	*/
	...
	ctx.on("session/event", (session, event) => {
		...
		if (isSubagentDescriptorEvent(event)) replay.send(binding.chatId, { markdown: subagentLine(event.data) }).catch(reportSendFailure);
