...
function installBridge(ctx, config, port, notify, authorization) {
	...
	*/
	...
	const bridgeDriving = /* @__PURE__ */ new Set();
	/** Latest external assistant text per bound session, flushed at turn/end. */
	const externalAnswers = /* @__PURE__ */ new Map();
	...
	* rule when it does not. */
	...
	const childIdFor = (tracker, agentId) => {
		if (agentId !== "" && tracker.entries.has(agentId)) return agentId;
		const running = runningEntries(tracker);
		return running.length === 1 ? running[0]?.id : void 0;
	};
	const noteChildActivity = (chatId, agentId, frame) => {
		...
		if (streamDebug) notify(`dsh-lark-bridge: child frame agent=${agentId} -> ${id ?? "unresolved"} (${runningEntries(tracker).length} running)`);
		if (id === void 0) return;
		if (markActivity(tracker, id, text) === void 0) return;
		flushSubagentCard(chatId, tracker, false).catch(reportSendFailure);
	};
	/**
	...
	*/
	...
	ctx.on("session/event", (session, event) => {
		...
		if (isAssistantMessageEvent(event) && !bridgeDriving.has(session.id)) {
			const text = event.data.message.content.filter((block) => block.type === "text" && block.text).map((block) => block.text).join("").trim();
			if (text !== "") externalAnswers.set(session.id, text);
		}
		if (isTurnEndEvent(event)) {
			bridgeDriving.delete(session.id);
			const external = externalAnswers.get(session.id);
			if (external !== void 0 && !bridgeDriving.has(session.id)) {
				externalAnswers.delete(session.id);
				replay.send(binding.chatId, { markdown: `🌐【web】${external.slice(0, 3e3)}${external.length > 3e3 ? "…" : ""}` }).catch(reportSendFailure);
			}
		...
		if (isSubagentCatalogEvent(event)) upsertSubagent(binding.chatId, event.data.childId, {
