...
async function accountReply(ctx, rest) {
	...
	if (action === void 0) {
		...
		const rows = names.map((key) => {
			...
			return `- **${key}**：\`${maskSecret(account.appId)}\`（存于 ${account.savedAt}${account.note === void 0 ? "" : `，${account.note}`}）${activeTag}`;
		});
		return {
			reply: `**账号库**（${syncDirHint()} 旁 accounts.json）\n${rows.join("\n")}\n\n切换：\`/bot account use <名字>\`；删除：\`/bot account forget <名字>\`。`,
			resolved: true
		};
	}
	if (action === "save") {
	...
	if (action === "use") {
		...
		if (account === void 0) return {
			reply: `⚠️ 账号库里没有 **${name}**。已有：${Object.keys(book.accounts).map((key) => `\`${key}\``).join(" / ") || "（空）"}`,
			resolved: false
		};
		await updateSettings(ctx.home, (current) => ({
			...current,
	...
	if (action === "forget") {
		if (name === void 0 || name === "" || book.accounts[name] === void 0) return {
			reply: "⚠️ 格式：`/bot account forget <名字>`（须是已存账号）",
			resolved: false
		};
