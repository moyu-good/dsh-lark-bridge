 ...
 */
export function deepseekBillingPhase(now = new Date()) {
    const { weekdayShort, minutes } = beijingParts(now);
    if (!WEEKDAYS.has(weekdayShort))
        return 'off-peak';
...
export function beijingClock(now = new Date()) {
    const { weekdayShort, minutes } = beijingParts(now);
    const names = { Mon: '一', Tue: '二', Wed: '三', Thu: '四', Fri: '五', Sat: '六', Sun: '日' };
 ...
 */
export function pricingPromptLine(now = new Date()) {
...
export function pricingHumanLine(now = new Date()) {
    const phase = deepseekBillingPhase(now);
