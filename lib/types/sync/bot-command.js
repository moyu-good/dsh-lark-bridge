...
import { buildMigration, buildImportPlan, crossHostWarning, ensureDeviceId, patchDeviceState, readDeviceState, readMigration, resolveMigrationFile, SECRET_KEYS, validateMigration } from "./migrate.js";
import { readAccounts, writeAccounts } from "./accounts-store.js";
import { buildAccountCard } from "../account-card.js";
import { FeishuCloud } from "./feishu-cloud.js";
 ...
 */
export function endpointKey(deviceId, form, profile) {
 ...
 */
export function arbitrationActiveKey(arbitration) {
    if (typeof arbitration.activeEndpoint === 'string')
        return arbitration.activeEndpoint;
 ...
 */
export function isActiveEndpoint(arbitration, deviceId, form, profile) {
    if (typeof arbitration.activeEndpoint === 'string') {
...
export async function readCloudArbitration(ctx) {
    const cloud = cloudOf(ctx);
    if (cloud === null)
        return null;
    try {
...
export async function arbitrationForInbound() {
...
export async function runBotCommand(line, ctx) {
    ...
    if (subcommand === 'export')
        return exportReply(ctx, rest);
    if (subcommand === 'import')
        return importReply(ctx, rest);
    if (subcommand === 'devices')
        return devicesReply(ctx);
    if (subcommand === 'retire')
        return retireReply(ctx);
    if (subcommand === 'activate')
        return activateReply(ctx);
    if (subcommand === 'name')
        return nameReply(ctx, rest);
    if (subcommand === 'account')
        return accountReply(ctx, rest);
    return {
...
async function importFromFile(ctx, file, apply, nameArg, origin) {
    ...
    for (const [profile, traveled] of Object.entries(file.profiles)) {
        ...
        plans.push({ profile, plan: buildImportPlan(local, profile, traveled) });
    }
    ...
    for (const { profile, plan } of plans) {
        ...
        for (const step of result.ran)
            lines.push(`  - ✅ ${step.spec}`);
        for (const failure of result.failures)
            lines.push(`  - ⚠️ ${failure.step.spec}：${failure.error}`);
    }
    if (reenter.length > 0) {
 ...
 */
export async function renewPresence(ctx) {
    const cloud = cloudOf(ctx);
    if (cloud === null)
        return;
    try {
 ...
 */
export async function accountUseByName(ctx, name) {
    const book = await readAccounts(ctx.home);
...
export async function accountForgetByName(ctx, name) {
    const book = await readAccounts(ctx.home);
