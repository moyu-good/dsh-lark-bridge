...
import { runBotCommand } from "./sync/bot-command.js";
import { fetchDeepseekBalance, formatBalanceReply } from "./balance.js";
import { USER_GUIDE } from "./user-guide.js";
 ...
 */
export let scheduleRestart = (shell) => {
...
export function setRestartScheduler(fn) {
 ...
 * @returns the markdown listing.
 */
export function helpText(commands, agent, locale = 'zh', config) {
    const own = [
 ...
 */
export async function runCommandLine(line, agent, commands, signal, presets = undefined, persistence = undefined, chatId = undefined, deniedTools = undefined, schedules = undefined, audits = undefined, config = undefined, sessionPresets = undefined, sessionQuery = undefined, jobs = undefined, feedback = undefined, lastAssistantMessageId = undefined, tokenMeter = undefined, skills = undefined, defaultModel = undefined, configModel = undefined, workspaces = undefined, currentCwd = undefined, loaderEntries = undefined, sync = undefined) {
 ...
 */
async function runSessionsCommand(agent, persistence, chatId, query = '', sessionQuery = undefined) {
    if (chatId === undefined) {
...
async function runSkillsCommand(line, skills) {
    ...
    const row = (s) => `· \`${s.name}\` — ${s.description}${s.source === undefined ? '' : `（${s.source}）`}`;
