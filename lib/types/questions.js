...
function questionActionValue(value) {
    if (typeof value !== 'object' || value === null)
        return undefined;
    const record = value;
    if (record.kind !== QUESTION_ACTION)
        return undefined;
 ...
 */
export function createQuestionProvider(port, chatFor) {
    ...
    const handleCardAction = (evt) => {
        ...
        const answers = item.questions.map(q => {
            ...
            if (value.option === '')
                return { id: q.id, selected: [], custom: '已确认' };
            const isOption = (q.options ?? []).some(o => o.label === value.option);
            if (!isOption)
                return { id: q.id, selected: [], custom: value.option };
