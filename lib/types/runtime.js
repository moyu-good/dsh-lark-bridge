...
import { detectRuntimeForm, defaultProfileFor } from "./form.js";
...
function withDeadline(work, ms, what) {
    return new Promise((resolve, reject) => {
        const timer = setTimeout(() => reject(new Error(`dsh-lark-bridge: ${what} timed out after ${ms}ms`)), ms);
        timer.unref?.();
...
export function apply(ctx, config) {
    ...
    const startSyncLayer = (resolved) => {
        ...
        void startControlApi({
            ...
            const renewPresenceTimer = setInterval(() => void import("./sync/bot-command.js").then((m) => m.renewPresence(syncContext)).catch(() => { }), 60_000);
            renewPresenceTimer.unref?.();
            void import("./sync/bot-command.js").then((m) => m.renewPresence(syncContext)).catch(() => { });
