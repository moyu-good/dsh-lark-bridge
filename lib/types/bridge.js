...
import { AUDIT_COMMAND, CONFIG_COMMAND, HELP_COMMAND, isCommandLine, JOBS_COMMAND, PRESET_COMMAND, runCommandLine, SCHEDULES_COMMAND, SESSIONS_COMMAND, STOP_COMMAND, TOOLS_COMMAND, } from "./commands.js";
...
import { compactionPruneLine, compactionSummaryLine, jobDoneLine, retryLine, scheduleLine, subagentEndLine, subagentLine, webSearchLine, } from "./notices.js";
    ...
    const handleMessage = async (msg) => {
        ...
        try {
            ...
            if (isCommandLine(msg.content)) {
                ...
                const outcome = await runCommandLine(msg.content, opened.handle.agent, ctx.get('commands'), commandSignal(), ctx.get('agentPresets'), ctx.get('sessionPersistence'), msg.chatId, runtimeDeniedTools, scheduleRegistry, auditStats, config, sessionPresets, ctx.get('sessionQuery'), ctx.get('jobs'));
