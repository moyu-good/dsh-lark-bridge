...
import { AUDIT_COMMAND, CONFIG_COMMAND, CONTEXT_COMMAND, FEEDBACK_COMMAND, HELP_COMMAND, isCommandLine, JOBS_COMMAND, MODEL_COMMAND, PLUGINS_COMMAND, PRESET_COMMAND, RESTART_COMMAND, runCommandLine, scheduleRestart, SCHEDULES_COMMAND, SESSIONS_COMMAND, SKILLS_COMMAND, STOP_COMMAND, TOOLS_COMMAND, WS_COMMAND, } from "./commands.js";
 ...
 */
function composeChatAgent(agentCtx, config, extraTools = [], runtimeDenied = undefined) {
...
export function installBridge(ctx, config, port, notify, authorization) {
    ...
    const turnWatchdog = new Map();
    const clearTurnWatchdog = (sessionId) => {
    ...
    const armTurnWatchdog = (sessionId, chatId) => {
        ...
        const timer = setTimeout(() => {
            ...
            if (config.restartCommand !== '')
                scheduleRestart(config.restartCommand);
        ...
        try {
            ...
            const opened = await Promise.race([
                ...
                new Promise((_, reject) => {
                    const timer = setTimeout(() => reject(new Error(ACQUIRE_HUNG_MARKER)), ACQUIRE_TIMEOUT_MS);
                    ...
                    acquireFor.then(() => clearTimeout(timer), () => clearTimeout(timer));
                }),
            ]);
            ...
            if (isCommandLine(msg.content)) {
                ...
                }, ctx.get('workspaceRegistry'), cwd, loaderEntries());
            ...
            if (imagesEnabled && visionPrefixes !== undefined && visionPrefixes.length > 0) {
                ...
                if (!visionPrefixes.some((prefix) => currentModel.startsWith(prefix))) {
                    imagesEnabled = false;
                    imagesNote = '当前模型不接受图片（visionModelPrefixes 未命中），用 /model 切到视觉模型后可发图';
                }
            }
            const images = await collectImages(msg, port, ctx.get('attachments'), imagesEnabled, imagesNote);
